...
void parserInitRules(){
	...
	parserAddRule(27, SYM_FUNC_BODY, SYMBOL_LPAREN, SYM_FUNC_ARG, SYMBOL_RPAREN); 	// func-body -> ( func-arg )
			...
			if(!isTerminal(i)){
				...
				for(int j = 0; j < RULE_N; j++){
					...
					if(parserRules[j].goal == i){
						...
						bool epsilonInPrev = parserSetHasEpsilon(parserFirstSet[parserRules[j].expr[0]]);
						allHasEpsilon &= epsilonInPrev;
						// Union first set of first expression excluding epsilon
						setChanged |= parserUnionSetComplEpsilon(parserFirstSet[parserRules[j].expr[0]], parserFirstSet[i]);
						...
						for(int k = 1; k < EXPR_N; k++){
							...
							if(allHasEpsilon){
								...
								setChanged |= parserUnionSetComplEpsilon(parserFirstSet[parserRules[j].expr[k]], parserFirstSet[i]);
							}

							// Update variables
							epsilonInPrev = parserSetHasEpsilon(parserFirstSet[parserRules[j].expr[k]]);
							allHasEpsilon &= epsilonInPrev;
						}
					}
				}
				...
				for(int j = 0; j < RULE_N; j++){
					...
					if(parserRuleHasTokenInExpr(j, i)){
						for(int k = 0; k < EXPR_N; k++){
							...
							if(k + 1 < EXPR_N){
								// Foreach X -> i beta
								if(parserRules[j].expr[k] == i && parserRules[j].expr[k+1] != -1){
									bool betaHasEpsilon = parserSetHasEpsilon(parserFirstSet[parserRules[j].expr[k+1]]);
									setChanged |= parserUnionSetComplEpsilon(parserFirstSet[parserRules[j].expr[k+1]], parserFollowSet[i]);

									// If epsilon is in Fi(beta)
									if(betaHasEpsilon){
										setChanged |= parserUnionSetComplEpsilon(parserFollowSet[parserRules[j].goal], parserFollowSet[i]);
									}
								}
		...
		for(int j = 0; j < CHAR_NUL; j++){
			...
			if(parserFirstSet[parserRules[i].expr[k]][j]){
				if(parserPredictTable[parserRules[i].goal][j] != i && parserPredictTable[parserRules[i].goal][j] != -1 && parserPredictTable[parserRules[i].goal][j] < RULE_N){
					...
					printf("[PSGEN] Conflict in parseTable[%17s][%17s] at rules:\n", tokenTypeName[parserRules[i].goal + 1], tokenTypeName[j + 1]);
					parserDisplayRule(parserPredictTable[parserRules[i].goal][j]);
					parserDisplayRule(i);
					parserStateOk = false;
			...
			if(parserSetHasEpsilon(parserFirstSet[parserRules[i].expr[k]]) && parserFollowSet[parserRules[i].goal][j] && parserPredictTable[parserRules[i].goal][j] < RULE_N){
				if(parserPredictTable[parserRules[i].goal][j] != i && parserPredictTable[parserRules[i].goal][j] != -1){
					printf("[PSGEN] Parse table conflict!\n");
					printf("[PSGEN] Conflict in parseTable[%17s][%17s] at rules:\n", tokenTypeName[parserRules[i].goal + 1], tokenTypeName[j + 1]);
					parserDisplayRule(parserPredictTable[parserRules[i].goal][j]);
					parserDisplayRule(i);
...
void parserDisplayRule(int index){
	printf("[PSGEN] Rule #%02d: %17s -> %s", index, tokenTypeName[parserRules[index].goal + 1], tokenTypeName[parserRules[index].expr[0] + 1]);
